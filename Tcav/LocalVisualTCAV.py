...
from Torch_VisualTCAV.utils import cosine_similarity, nth_highest_index, contraharmonic_mean
...
#---TO TEST---
	...
	def __init__(
		...
		*args, **kwargs
		...
		img = Image.open(self.test_images_dir).convert('RGB')  # Carica e converte in RGB
		resize_transform = transforms.Resize(self.resized_imgs_size, interpolation=Image.BILINEAR)
		tensor_transform = transforms.ToTensor()

		# Applica le trasformazioni di ridimensionamento e converte in tensor
		resized_img = resize_transform(img)
		resized_img_tensor = tensor_transform(resized_img).unsqueeze(0)  # Aggiunge una dimensione per il batch

		# Se hai una directory con più immagini, puoi usare un ciclo per caricarle tutte
		self.imgs = []
		self.resized_imgs = []
		for image_file in os.listdir(test_images_dir):
				img_path = os.path.join(test_images_dir, image_file)
				img = Image.open(img_path).convert('RGB')
				self.imgs.append(tensor_transform(img))
				resized_img = resize_transform(img)
				self.resized_imgs.append(tensor_transform(resized_img))

		self.imgs = torch.stack(self.imgs)
		self.resized_imgs = torch.stack(self.resized_imgs)
        ...
        for concept_name in self.concepts:
            ...
            if not cav_only:
                ...
                if emblem_max > emblem_min:
                    concept_layer.concept_map = torch.clamp(concept_layer.concept_map, min=emblem_min, max=emblem_max)
            ...
            for n_class in range(self.n_classes):
                ...
                if self.model.binary_classification:
                    ...
                    if n_class == 0:
                        ...
                        attributions[n_class] = attributions[n_class] * (virtual_logit_0 / (attributions[n_class].sum() + 1e-10))
                    else:
                        attributions[n_class] = F.relu(-binary_attributions)
                        attributions[n_class] = attributions[n_class] * (virtual_logit_1 / (attributions[n_class].sum() + 1e-10))
                else:
                ...
                for n_class in range(self.n_classes):
                    ...
                    if feature_maps.min() < 0:
                        pooled_cav_norm = F.relu(
                            ...
                            torch.where((feature_maps * self.computations[layer_name][concept_name].concept_map[:, :, None]).sum(dim=(0, 1)) < 0, -1.0, 1.0)
                        )
                    else:
	...
	def plot(self, paper=False):
		...
		for concept_name in self.concepts:
			...
			if not paper:
				...
				fig.suptitle(f"$\mathit{{{model_name_esc}}}$ architecture\n$\mathit{{{concept_name_esc}}}$ concept", fontsize=10+1.5*len(self.layers))
			else:
			...
			for j, layer_name in enumerate(self.layers):
				...
				heatmap = F.interpolate(
						...
						size=(self.imgs[0].shape[0], self.imgs[0].shape[1]),
						mode='bilinear',
						align_corners=False
				)
				...
				heatmap = np.array(PIL.Image.fromarray(np.uint8(heatmap * 255) , 'L')
                                  .filter(PIL.ImageFilter.GaussianBlur(radius = 21)))  / 255
				...
				plt.title(f"$\mathit{{{layer_name_esc}}}$ {layer_description}", fontsize=9+1.5*len(self.layers), y=0.95)
				plt.tight_layout()
				rows = []
